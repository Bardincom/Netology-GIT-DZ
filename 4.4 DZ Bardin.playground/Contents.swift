...
class Category {
  ...
  func removeTrack(number: Int) {
    // провеяем содержит ли данный массив указанный индекс Трека, если нет такого Индекса выдает сообщение про ошибку
    guard trackList.indices.contains(number - 1) else {
      print("ERROR: Введенный номер трека отсутствует")
      return
    }
...
categoryRock.removeTrack(number: 2)
...
class Library {
  ...
  func transferTrack(from categoryOne: Category,
    ...
    guard categoryOne.trackList.indices.contains(number - 1) else {
           print("ERROR: Запрашиваемый трек для переноса не найден")
           return
       }
